...
suite('vscode API - webview', () => {
	...
	test('loads many local resources concurrently without crashing', async function () {
		this.timeout(vscode.env.uiKind === vscode.UIKind.Desktop ? 60_000 : 120_000);
		...
		try {
			...
			const didDispose = asPromise(panel.onDidDispose);
			...
			const resources = await Promise.all(Array.from({ length: resourceCount }, async (_, index) => {
