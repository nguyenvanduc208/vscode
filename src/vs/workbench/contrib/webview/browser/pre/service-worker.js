...
async function processResourceRequest(
	...
	const resolveResourceEntry = (result, cachedResponse, releaseHostResourceResponseBody) => {
		...
		if (entry.stream) {
			...
			if (shouldTryCaching && entry.etag) {
				const releaseAfterStreamAndCache = createJoinedFinalizer(releaseHostResourceResponseBody, 2);
				const trackedStream = trackReadableStreamLifetime(entry.stream, releaseAfterStreamAndCache);
				const [responseStream, cacheStream] = trackedStream.tee();
				...
				void caches.open(resourceCacheName)
					...
					.finally(releaseAfterStreamAndCache);
				return response;
			}
	...
	const releaseHostResourceResponseBody = webviewId
